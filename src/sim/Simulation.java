...
public class Simulation extends PApplet {
	...
	protected void load(){
		load( new File( this.root.getAbsolutePath() + File.separator + "tests" + File.separator + testName ) );
	}
	protected void load( int session ){
		load( new File( this.root.getAbsolutePath() + File.separator + "outputs" + File.separator + testName + File.separator + "session " + session ) );
	}
	private void save(ArrayList<ArrayList<Pitch>> pitches){
		if( changeHappened ){
			if( pitches.size() > 1 ){
				String root = this.root.getAbsolutePath() + "\\outputs\\" + testName + "\\";
	...
	public void draw(){
		...
		fill(0,50,50,200);
		rect(width-TRANSPORT_WIDTH,0,TRANSPORT_WIDTH/2,3*textAscent());
		fill(280,50,50,200);
		rect(width-TRANSPORT_WIDTH/2,0,TRANSPORT_WIDTH/2,3*textAscent());
		fill(0,0,255);
		...
		text("Group: "+comp.groupSize(),width-TRANSPORT_WIDTH,3*textAscent());
		updateNoteQueue();
	...
	public void mouseReleased(){
		...
		if( mouseY < h ){
			if (mouseX < width - TRANSPORT_WIDTH ){
				...
				addNumbered( (int) f );
				selectedBuffer = 0;
			} else if( mouseY > 2*textAscent() && mouseY < 3*textAscent() ){
				comp.setGroupSize( comp.groupSize() + ((mouseX> width - TRANSPORT_WIDTH/2) ? 1 : -1) );
			}
		} else {
			int i = (int)((mouseY-h)/h * noteQueue.size());
			if( i >= 0 && i < noteQueue.size() ) selectedBuffer = i;
			if( mouseButton == LEFT ){
				playSelected();
				if( selectedBuffer != 0 ){
					save( noteQueue );
	...
	public static void main(String[] args) {
		PApplet.main( new String[] {"sim.Simulation", "--present"} );
	}
